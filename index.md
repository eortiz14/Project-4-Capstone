...
Looking back, we built a great model, but maybe we missed 2 specific topics.This are the gender distribution in the database and the possible option of using fewer variables without significantly affecting the metrics model. 
...
Using fewer variables without significantly affecting the metrics of the model is a good idea thinking to optimize resources and make more easy deployment that usually was made on plataforms like dataiku or datarobot. So in the next steps we could use iterative techniques like Forward Feature Selection,Backward Feature Elimination or Exhaustive Feature Selection to select less features and obtain similar results.
